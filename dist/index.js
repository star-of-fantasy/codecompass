...
async function listWorkspaceRoots() {
    ...
    if (envRoot) {
        ...
        uniqueRoots.set(normalizeComparisonPath(resolvedEnvRoot), resolvedEnvRoot);
    }
    const cwdRoot = path.resolve(process.cwd());
    uniqueRoots.set(normalizeComparisonPath(cwdRoot), cwdRoot);
    return [...uniqueRoots.values()];
}
async function resolveTargetPath(relativePath) {
...
function toEditorDeepLink(absolutePath, lineNumber) {
...
function toExtensionOpenLink(workspaceRoot, relativePath, lineNumber) {
    const extensionId = process.env.CODECOMPASS_VSCODE_EXTENSION_ID?.trim() ||
        "codecompass.codecompass-preview";
    const safeLine = Math.max(1, lineNumber);
    const safePath = relativePath.replace(/\\/g, "/");
    const query = `path=${encodeURIComponent(safePath)}&line=${encodeURIComponent(String(safeLine))}&root=${encodeURIComponent(workspaceRoot)}`;
    return `vscode://${extensionId}/open?${query}`;
}
function toSplitCommandUri(absolutePath, lineNumber) {
...
function buildMarkdownLink(symbolName, workspaceRoot, absolutePath, lineNumber) {
    const fileName = path.basename(absolutePath);
    const safeLine = Math.max(1, lineNumber);
    const mode = getLinkMode();
    const href = (() => {
        if (mode === "command")
            return toSplitCommandUri(absolutePath, lineNumber);
        if (mode === "extension") {
    ...
    const title = (() => {
        if (mode === "command")
            return `在右侧分栏打开 ${fileName} 第${safeLine}行`;
        if (mode === "extension")
...
server.tool("create_split_link", "将代码符号（函数名/变量名/文件名）的位置转换为 VS Code Markdown 可点击链接。" +
    ...
    const link = buildMarkdownLink(symbol_name, resolvedTarget.workspaceRoot, resolvedTarget.absolutePath, line_number);
    return {
        content: [
            {
                type: "text",
...
server.tool("create_split_links_batch", "批量生成多个代码符号的可点击链接，减少多次调用开销。" +
    ...
    for (const sym of symbols) {
        ...
        if (!resolvedTarget) {
            results.push(`- [错误] 路径 "${sym.relative_path}" 超出工作区范围`);
            continue;
        }
        results.push(buildMarkdownLink(sym.symbol_name, resolvedTarget.workspaceRoot, resolvedTarget.absolutePath, sym.line_number));
    }
    return {
        content: [
            {
