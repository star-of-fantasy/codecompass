...
function parseArgs(argv: string[]): InstallOptions | null {
  ...
  for (let index = 0; index < rest.length; index += 1) {
    ...
    if (arg === "--workspace") {
      if (!next) throw new Error("--workspace 需要提供目录路径");
...
function validateOptions(options: InstallOptions): void {
...
function getCursorRoot(options: InstallOptions): string {
...
function getMcpConfigPath(options: InstallOptions): string {
  return path.join(getCursorRoot(options), "mcp.json");
}

function getSkillTargetDir(options: InstallOptions): string {
...
function findVsixPath(): string {
  const fixedAsset = path.join(packageRoot, "assets", "codecompass-preview.vsix");
  if (fs.existsSync(fixedAsset)) return fixedAsset;
  ...
  if (fs.existsSync(extensionDir)) {
    const candidates = fs
      ...
      .map((fileName) => {
        const match = /^codecompass-preview-(\d+\.\d+\.\d+)\.vsix$/.exec(
          fileName
        );
      ...
      .filter(
        (item): item is { version: string; filePath: string } => item !== null
      )
  ...
  throw new Error(
    "未找到 CodeCompass Preview 插件 VSIX，请先执行 npm run package:extension"
  );
}
...
function installExtension(options: InstallOptions): void {
  ...
  if (result.error) {
    throw new Error(
      `无法执行 ${options.editor} CLI，请确认它已加入 PATH。\n${result.error.message}`
    );
  }
...
function isNpxTempInstall(): boolean {
  return packageRoot.split(path.sep).some((part) => part === "_npx");
}

function resolveMcpCommandConfig(): McpCommandConfig {
  if (isNpxTempInstall()) {
...
function installMcpConfig(options: InstallOptions): string {
...
function backupDirectory(targetDir: string): void {
...
function installSkill(options: InstallOptions): string {
...
function runInstall(options: InstallOptions): void {
